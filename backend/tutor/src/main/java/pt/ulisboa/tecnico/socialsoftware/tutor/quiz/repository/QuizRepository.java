...
public interface QuizRepository extends JpaRepository<Quiz, Integer> {
    ...
    @Query(value = "SELECT * FROM {h-schema}quizzes q, {h-schema}course_executions c WHERE c.id = q.course_execution_id AND c.id = :executionId ORDER BY c.id", nativeQuery = true)
    ...
    @Query(value = "SELECT q.course_execution_id FROM {h-schema}q WHERE q.id = :id", nativeQuery = true)
