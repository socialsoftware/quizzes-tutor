...
public interface QuizRepository extends JpaRepository<Quiz, Integer> {
    ...
    @Query(value = "SELECT * FROM quizzes q, course_executions c WHERE c.id = q.course_execution_id AND c.id = :executionId AND q.available_date < :now AND (q.conclusion_date IS NULL OR q.conclusion_date > :now)", nativeQuery = true)
    List<Quiz> findAvailableQuizzes(int executionId, LocalDateTime now);
