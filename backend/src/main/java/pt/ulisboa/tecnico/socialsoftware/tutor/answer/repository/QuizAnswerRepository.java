...
public interface QuizAnswerRepository extends JpaRepository<QuizAnswer, Integer> {
    ...
    @Query(value = "SELECT q FROM QuizAnswer qa JOIN qa.quiz q WHERE qa.user.id = :userId AND q.courseExecution.id = :executionId AND q.qrCodeOnly = true AND (qa.completed = false AND (q.oneWay = false OR qa.creationDate IS NULL))")
    Stream<Quiz> findOpenQRCodeQuizzes(int userId, int executionId);
