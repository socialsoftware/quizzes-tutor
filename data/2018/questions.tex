...
\begin{ClosedQuestion}
  ...
  \begin{options}
    \option Não faz sentido usar vistas do tipo de vista módulo, uma
    ...
    \option Pode ser necessário usar vistas dos três tipos de vistas
    \option As únicas vistas que são relevantes para aspectos de
...
\begin{ClosedQuestion}
  ...
  \begin{options}
    ...
    desenvolver
    \option Devem ser capturados através de cenários, tais como os
    requisitos de atributos de qualidade, e ser tidos em conta no
    desenho da arquitectura de software
    \option São importantes para determinar a viabilidade do sistema,
    ...
    \option Não são uma das influências da arquitectura de software no
...
\begin{ClosedQuestion}
  ...
  \begin{options}
    \option A camada 1
    \option A camada 4
    \option Numa arquitectura de camadas, qualquer camada é igualmente
    modificável
    \option A facilidade de modificação não é particularmente
...
\begin{ClosedQuestion}
  ...
  \begin{options}
    \option Só é útil se for feito (ainda que apenas parcialmente)
    ...
    \option É útil mesmo que a implementação já tenha sido concluída e
...
\begin{ClosedQuestion}
  ...
  \begin{options}
    \option Um subconjunto dos requisitos que não entram em conflito
    ...
    \option Um subconjunto dos requisitos que é escolhido
...
\begin{ClosedQuestion}
 ...
 Tendo em conta esta descrição simplificada do funcionamento do
  ...
  \begin{options}
    ...
    \option O estilo Publicação-Subscrição (\emph{Publish-subscribe})
 \end{options}
\end{ClosedQuestion}
}
...
\begin{ClosedQuestion}
  ...
  \begin{options}
    \option O estilo Processos Comunicantes (\emph{Communicating Processes})
    \option O estilo Utilização (\emph{Uses})
...
\begin{ClosedQuestion}
  ...
  \begin{options}
    \option Vistas de Decomposição (\emph{Decomposition}) e de Camadas (\emph{Layers})
    \option Vistas de Decomposição (\emph{Decomposition}) e de
    Atribuição de Trabalho (\emph{Work assignment})
    \option Vistas de Decomposição (\emph{Decomposition}) e de Implementação (\emph{Implementation})
    \option Vistas de Decomposição (\emph{Decomposition}) e de Generalização (\emph{Generalization})
  \end{options}
\end{ClosedQuestion}
}
...
\begin{ClosedQuestion}
  ...
  \begin{options}
    \option Vistas de Decomposição (\emph{Decomposition}) e de Implementação (\emph{Implementation})
    \option Vistas de Decomposição (\emph{Decomposition}) e de Utilização (\emph{Uses})
...
\begin{ClosedQuestion}
  ...
  \begin{options}
    \option O estilo Dados partilhados (\emph{Shared data})
    \option O estilo Repositório (\emph{Repository})
    \option O estilo Camadas (\emph{Layers})
    \option O estilo Cliente-Servidor (\emph{Client-Server})
  \end{options}
\end{ClosedQuestion}
...
\begin{ClosedQuestion}
  ...
  \begin{options}
    \option Os estilos Decomposição (\emph{Decompostion}) e Implementação (\emph{Implementation})
    \option Os estilos Instalação (\emph{Deployment}) e Utilização (\emph{Uses})
    \option Os estilos Cliente-Servidor (\emph{Client-Server}) e Generalização (\emph{Generalization})
    \option Os estilos Instalação (\emph{Deployment}) e Camadas (\emph{Layers})
  \end{options}
\end{ClosedQuestion}
}
...
\begin{ClosedQuestion}
  ...
  \begin{options}
    ...
    código de modo a alcançar os níveis de desempenho necessários actualmente
    \option Alterar a vista de instalação do sistema, replicando o servidor por
    mais máquinas
    ...
    \option Substituir a máquina usada pelo servidor por uma máquina
    mais poderosa que satisfaça os novos requisitos de desempenho,
    mantendo apenas um componente servidor em funcionamento
  \end{options}
\end{ClosedQuestion}
...
\begin{ClosedQuestion}
  ...
  \begin{options}
    ...
    \option Vistas do tipo de vista módulo
    \option Vistas do tipo de vista afectação (\emph{Allocation})
...
\begin{ClosedQuestion}
  ...
  \begin{options}
    \option Controlar e reduzir a interface exposta pela camada de lógica de
    domínio, aumentando assim a modificabilidade dessa camada
    \option Permitir a existência de mais do que uma camada de lógica
    de apresentação para a mesma aplicação (para providenciar, por
    exemplo, uma interface de \emph{web services})
    \option Expor diferentes interfaces da camada de lógica de domínio
    para permitir a existência de diferentes camadas de lógica de apresentação
    \option Facilitar o utilização da camada de acesso a dados por
    parte da camada de lógica de apresentação
  \end{options}
\end{ClosedQuestion}
}
...
\begin{ClosedQuestion}
  ...
  \begin{options}
    \option Implementar cada um dos serviços que são executados sempre
    ...
    feito pelo cliente
    \option Dividir a computação necessária para processar cada um dos
    pedidos feitos pelo cliente em unidades de trabalho mais pequenas
    e paralelizáveis
    \option Manter um registo das alterações feitas aos dados durante
...
\begin{ClosedQuestion}
  Existem vários factores que podem influenciar o desenho da
  arquitectura de software de um sistema, mas, tanto a arquitectura,
  como o sistema desenvolvido com base nessa arquitectura, também
  podem influenciar o ambiente que lhe deu origem.  A isto chamamos o
  Ciclo de Negócio da Arquitectura (\emph{Architecture Business
    Cycle}).  Qual dos seguintes aspectos é mais raramente
  influenciado pela arquitectura?

  \begin{options}
    \option A experiência do arquitecto de software
    \option O ambiente tecnológico para o desenvolvimento de novos sistemas
    \option A estrutura da organização que desenvolve o sistema
...
\begin{ClosedQuestion}
  A descrição da arquitectura de software de um sistema tem, entre
  outras, a função de mostrar que o sistema satisfaz cada um dos
  requisitos que foram identificados para o sistema (por exemplo, um
  requisito de disponibilidade).  Que relação existe entre as vistas
  de uma arquitectura de software e os requisitos?

  \begin{options}
    \option Cada vista mostra a satisfação de um só requisito e esse
    requisito fica completamente satisfeito através dessa vista
    \option Basta uma vista para mostrar a satisfação de um requisito,
    mas uma vista pode mostrar a satisfação de mais do que um requisito
    \option São necessárias várias vistas para mostrar a satisfação de
    um requisito, mas cada vista apenas trata de um tipo de requisitos
    (embora possam ser vários do mesmo tipo)
...
\begin{ClosedQuestion}
  Os cenários gerais desempenham um papel importante no
  desenvolvimento de uma arquitectura de software porque

  \begin{options}
    \option Identificam quais são os requisitos mais importantes que o
    sistema deve satisfazer
    \option Servem-nos de guia na elicitação de requisitos junto dos
    ...
    satisfaz requisitos menos específicos e que cobrem um leque maior
    de situações que podem ocorrer no sistema
  \end{options}
\end{ClosedQuestion}
...
\begin{ClosedQuestion}
  Uma decomposição por objectos de um módulo é mais adequada do que
  uma decomposição funcional quando

  \begin{options}
    \option Se prevê que a maior parte das evoluções do módulo sejam a
    adição de novas operações que o módulo deve fornecer
    \option Se prevê que a maior parte das evoluções do módulo sejam
    alterações ao comportamento das operações já existentes, por
    causa da necessidade de lidar com novos tipos de entidades
    \option A maior parte das operações do módulo têm necessidade de
...
\begin{ClosedQuestion}
  Suponha que é administrador de sistemas de uma empresa que
  desenvolve software, e que tem como tarefa preparar um sistema de
  backups regulares e automáticos para o código de um projecto de
  grande dimensão e importância para a empresa.  Qual das seguintes
  afirmações é mais correcta?

  \begin{options}
    \option Não existe nenhuma vista arquitectural que seja relevante
    para si
    \option A vista mais importante para o seu trabalho é a de
    Decomposição (\emph{Decomposition}), porque identifica as unidades
    de implementação do projecto
    \option A vista mais importante para o seu trabalho é a de
    Atribuição de Trabalho (\emph{Work Assignment}), porque lhe
    permite identificar quem são as pessoas envolvidas no projecto e
    assim saber que áreas de utilizadores devem ser salva-guardadas
    \option A vista mais importante para o seu trabalho é a de
    Implementação (\emph{Implementation}), porque descreve de que
    forma é que os artefactos do sistema estão organizados no sistema
    de ficheiros
  \end{options}
\end{ClosedQuestion}

\begin{ClosedQuestion}
  Um dos mais conhecidos fornecedores de soluções para \emph{Cloud
    Computing} é a Amazon, através do seu serviço \emph{Amazon Elastic
    amamozn (EC2)}.  O seguinte excerto foi retirado da página
 ...
 \end{quote}
 Supondo que pretende usar o serviço EC2 da Amazon descrito acima num
 sistema de software que está a desenvolver, que vista arquitectural é
 mais afectada por essa decisão?

  \begin{options}
    \option A vista do estilo Decomposição (\emph{Decomposition})
    \option A vista do estilo Instalação (\emph{Deployment})
    \option A vista do tipo Componente-e-Conector
    \option Nenhuma destas, uma vez que a utilização do EC2 não altera
    o código que vai ser desenvolvido
  \end{options}
\end{ClosedQuestion}
...
\begin{ClosedQuestion}
  ...
  \begin{options}
    ...
    \option Representam as relações de dependência existentes entre os
    vários componentes
    \option Representam os fluxos de controlo durante a execução do
    sistema
  \end{options}
\end{ClosedQuestion}
...
\begin{ClosedQuestion}
  Suponha que está a desenvolver um sistema de software para uma
  organização, e que esse sistema tem vários componentes a executar em
  diversos dispositivos dentro da organização, componentes esses que
  trocam informação entre si.  Suponha ainda que um dos requisitos de
  segurança para o sistema é que a informação manipulada pelo sistema
  não possa ser acedida por pessoas externas à organização (isto é,
  pessoas sem acesso às instalações da organização).  Para satisfazer
  este requisito

  \begin{options}
    \option Deve providenciar um módulo na vista de Decomposição
    (\emph{Decomposition}) que seja responsável pela segurança da
    informação
    \option Deve criar um componente que fique responsável pela
    autenticação dos utilizadores
    \option Deve exigir que os conectores utilizem protocolos seguros
    de comunicação
    \option Deve exigir, na vista de Instalação (\emph{Deployment}) do
...
\begin{ClosedQuestion}
  Suponha que está a implementar um módulo num sistema que tem uma
  arquitectura de duas camadas.  Sabendo que o seu módulo pertence à
  camada de cima (assumindo a notação habitual para o estilo camadas),
  isso significa que

  \begin{options}
    \option Pode utilizar operações definidas em qualquer módulo do
    sistema
    \option Pode utilizar operações definidas na camada de baixo,
    mas não as definidas na camada de cima
    \option Pode utilizar operações definidas na camada de cima,
    mas não as definidas na camada de baixo
    \option Deve utilizar alguma operação definida na camada de baixo
  \end{options}
\end{ClosedQuestion}


\begin{ClosedQuestion}
  Normalmente, os clientes de Instant Messaging têm uma janela com a
  lista de contactos do utilizador, e mostram nessa janela o estado de
  cada contacto (se está disponível, indisponível, ocupado, etc).
  Visto que o estado de um contacto pode ser alterado a qualquer
  instante, e que o estado de um contacto é fornecido pelo cliente de
  Instant Messaging desse contacto, qual é o estilo arquitectural que
  melhor representa o padrão de interacção entre estes componentes?
...
\begin{ClosedQuestion}
  Suponha que pertence à equipa de testes de uma empresa de
  desenvolvimento de software, sendo responsável por verificar o
  funcionamento correcto dos módulos desenvolvidos pela empresa para
  um determinado sistema.  Que vistas da arquitectura do sistema são
  mais úteis para realizar o seu trabalho

  \begin{options}
    \option As vistas do estilo Atribuição de Trabalho (\emph{Work Assignment})
    \option As vistas do estilo Decomposição (\emph{Decomposition})
    \option As vistas do estilo Utilização (\emph{Uses})
    \option As vistas do estilo Implementação (\emph{Implementation})
  \end{options}
\end{ClosedQuestion}


\begin{ClosedQuestion}
  No desenvolvimento de uma aplicação distribuída, desconfia que pode
  precisar de utilizar uma cache num dos componentes para reduzir o
  número de pedidos que esse componente tem que fazer aos restantes
  componentes do seu sistema, mas não quer ter que lidar com os
  problemas de manter uma cache se isso não for estritamente
  necessário.  Infelizmente, só quando o sistema estiver em produção é
  que será possível saber se a cache é mesmo necessária.  Sabendo que
  existem vários pontos no código desse componente onde é preciso
  invocar serviços remotos, qual é a arquitectura mais adequada para
  desenvolver este sistema?

  \begin{options}
    \option Uma arquitectura por camadas em que a camada mais abaixo
    providencia uma interface para invocação de serviços remotos
    ...
    código precisa de aceder
    \option Uma arquitectura em que definimos um módulo que tem como
    responsabilidade manter uma cache e que tem dois submódulos: um
    que implementa uma cache sempre vazia, e outro que implementa uma
    cache real
    \option Uma arquitectura do estilo Canais-e-Filtros
    (\emph{Pipes-and-filters}), em que as invocações de um serviço
    remoto são feitas através de uma cadeia de filtros, que pode
    incluir ou não a utilização de uma cache
  \end{options}
\end{ClosedQuestion}


\begin{ClosedQuestion}
  Suponha que para desenvolver uma aplicação empresarial (que
  necessita de aceder a uma base de dados) foi decidido usar a
  \emph{framework} Hibernate para facilitar o desenvolvimento do
  código de acesso aos dados.  Que vista arquitectural é mais adequada
  para representar esta decisão?

  \begin{options}
    \option Uma vista do estilo Instalação (\emph{Deployment})
    \option Uma vista do tipo Componente-e-Conector
    \option Uma vista do estilo Utilização (\emph{Uses})
    \option Uma vista do estilo Decomposição (\emph{Decomposition})
  \end{options}
\end{ClosedQuestion}


\begin{ClosedQuestion}
  A operadora de telecomunicações móveis Pessimus mantém uma base de
  dados onde regista todas as chamadas efectuadas pelos seus clientes.
  Cada registo tem o número do cliente, o número contactado, e tempos
  de início e de fim da chamada.  Para além disso, a Pessimus tem,
  naturalmente, uma outra base de dados onde tem informação completa
  sobre cada um dos seus clientes (nome, morada, número de
  contribuinte, etc).  Suponha que a Pessimus lhe pediu para
  desenvolver uma aplicação web que permita consultar os extractos
  detalhados de um cliente (um extracto detalhado mostra a lista de
  todas as chamadas efectuadas num determinado período).  Qual dos
  seguintes padrões é mais adequado para implementar a camada de
  acesso aos dados desta aplicação
 
  \label{q:pessimus}

  \begin{options}
    \option O padrão Table Data Gateway
    \option O padrão Row Data Gateway
    \option O padrão Active Record
    \option O padrão Data Mapper
  \end{options}
\end{ClosedQuestion}


\begin{ClosedQuestion}
  Considerando o cenário da aplicação a desenvolver para a Pessimus
  descrito na pergunta~\ref{q:pessimus}, qual dos seguintes padrões
  lhe parece mais adequado para ser aplicado no desenvolvimento desta
  aplicação

  \begin{options}
    \option O padrão Domain Model, porque permite representar os
    clientes e os registos das suas chamadas através de uma relação de
    um para muitos, facilitando assim a leitura dos dados necessários
    para uma operação
    \option O padrão Transaction Script, porque é o que permite o
    desenvolvimento mais rápido da aplicação
    \option O padrão Unit of Work, porque garante uma visão
    consistente dos dados das duas bases de dados numa única operação
    de negócio
    \option O padrão Identity Map, porque evita a existência de duas
    instâncias diferentes para representar o mesmo cliente
  \end{options}
\end{ClosedQuestion}


\begin{ClosedQuestion}
  A aplicação que desenvolveu para a Pessimus (ver
  pergunta~\ref{q:pessimus}), teve um enorme sucesso e é agora uma das
  aplicações mais usadas pelos clientes, tendo um número muito elevado
  de pedidos todos os dias.  Por isso, foi-lhe pedido que aumentasse o
  desempenho da aplicação.  Nomeadamente, que aumentasse o número de
  pedidos que a aplicação consegue processar por minuto.  Supondo que
  é aceitável que os extractos detalhados não mostrem as chamadas
  realizadas nas últimas 24 horas, a melhor solução para este problema
  é
  ...
  \begin{options}
    ...
    à base de dados
    \option Qualquer uma das opções acima é válida, dependendo de onde
    é que está o estrangulamento (bottleneck) do sistema
  \end{options}
\end{ClosedQuestion}
...
\begin{ClosedQuestion}
  Um dos objectivos do padrão Unit of Work é reduzir o número de
  acessos feitos à base de dados (BD).  Para isso, este padrão

  \begin{options}
    \option Mantém uma cache dos objectos lidos anteriormente para
    evitar fazer leituras repetidas da BD
    \option Permite ao programador definir um conjunto de objectos que
    irão ser necessários para a realização de uma determinada
    operação, e lê-os todos numa única operação de acesso à BD
    \option Mantém um registo dos objectos que foram alterados no
    decorrer de uma operação e faz os acessos à BD para efectuar
    essas alterações apenas no final da operação
    \option Ao carregar da BD um objecto que tem relações com outros
    objectos, carrega na mesma operação os objectos que são apontados
    pelo objecto carregado para reduzir a probabilidade de precisar de
    fazer novos acessos à BD a seguir
  \end{options}
\end{ClosedQuestion}


\begin{ClosedQuestion}
  Para implementar o padrão Identity Map é necessário manter um
  registo dos objectos lidos da base de dados anteriormente.  Que
  objectos do modelo de domínio da aplicação devem ser mantidos nesse
  registo?

  \begin{options}
    \option Apenas os objectos que correspondem a entidades
    (instâncias de \emph{entity types})
    \option Apenas os objectos que correspondem a valores (instâncias
    de \emph{value types})
    \option Todos os objectos que são criados a partir de dados
    armazenados na BD, sejam eles entidades ou valores
    \option Todos os objectos do modelo de domínio, quer tenham vindo
    da base de dados ou não
  \end{options}
\end{ClosedQuestion}


\begin{ClosedQuestion}
  O padrão Lazy Load é usado na implementação de um Data Mapper para
  controlar a forma como os objectos são lidos da base de dados (BD).
  O objectivo deste padrão é

  \begin{options}
    \option Reduzir o número de acessos feitos à BD para ler os
    objectos que são necessários para executar uma operação
    \option Reduzir o tempo gasto na leitura dos objectos necessários
    para executar uma operação
    \option Reduzir a memória gasta pelos objectos lidos para a
    execução de uma operação
    \option Permitir ler os objectos em paralelo com a execução da
    operação para reduzir o tempo em que se espera pela resposta da BD
 \end{options}
\end{ClosedQuestion}


\begin{ClosedQuestion}
  A arquitectura mais comum para uma aplicação web actualmente é
  composta por três componentes: cliente web, servidor web, e base de
  dados.  O servidor web, por sua vez, costuma ter uma arquitectura de
  três camadas: lógica de apresentação, lógica de domínio, e acesso a
  dados.  Suponha que tem uma aplicação web com esta arquitectura e
  que pretende fornecer uma interface de \emph{web services} para a
  sua aplicação (para além da interface web já existente).  Para isso,
  a melhor opção, tendo em conta o esforço de desenvolvimento é

  \begin{options}
    \option Adicionar uma nova camada de serviços entre a camada de
    lógica de domínio e a camada de apresentação já existentes, e
    fazer as alterações no código já existente que forem necessárias
    para respeitar esta nova arquitectura
    \option Criar uma nova camada acima da camada de lógica de negócio
    que implemente a interface de \emph{web services} pretendida, e
    usá-la num novo componente do tipo servidor que executa esta nova
    camada e as duas camadas de baixo do sistema original
    \option Criar um novo módulo que implemente os \emph{web
      services} e executar este módulo num novo componente do sistema
    que funcione como cliente do servidor web já existente
    \option Separar o componente servidor web em dois componentes, um
    que apenas tem a responsabilidade de servir pedidos de páginas
    web, e um servidor aplicacional que implemente a lógica de negócio
    do sistema
  \end{options}
\end{ClosedQuestion}
...
%% 1
\begin{ClosedQuestion}
  Supondo que está a desenvolver uma aplicação sozinho para depois a
  comercializar, então

 \begin{options}
    \option Não precisa de fazer o desenho da arquitectura de software
    do sistema, visto que está a fazer a aplicação sozinho
    \option Deve documentar a arquitectura de software do sistema,
    para o caso de precisar de envolver mais alguém na equipa de
    desenvolvimento 
    \option Deve documentar a arquitectura de software do sistema se
    isso ajudar a explicar as características da sua aplicação aos
    potenciais clientes da aplicação
    \option Não precisa de documentar a arquitectura de software do
    sistema, visto que o sistema não tem complexidade suficiente que o
    justifique
  \end{options}
\end{ClosedQuestion}
...
\begin{ClosedQuestion}
  ...
  sistema, muito provavelmente produzirão duas arquitecturas de
  software diferentes.  Se o sistema já se encontrar desenvolvido, mas
  sem documentação sobre a sua arquitectura, e os mesmos dois
  arquitectos tiverem como função documentar a arquitectura do sistema
  com base apenas no código do sistema

  \begin{options}
    \option Muito provavelmente produzirão duas arquitecturas de
    software diferentes, visto que o código não permite representar
    toda a informação arquitecturalmente relevante
    \option Muito provavelmente produzirão a mesma arquitectura de
    software, visto que o sistema já se encontra desenvolvido e não existem
    ambiguidades sobre o que ele faz
    \option Não é possível documentar a arquitectura de software de um
    sistema a partir apenas do seu código
    \option Não é relevante documentar a arquitectura de software do
    sistema se este já se encontra desenvolvido, visto que a
...
%% 3
\begin{ClosedQuestion}
  A arquitectura de software de um sistema é normalmente representada
  através de várias vistas diferentes porque

  \begin{options}
  \option Representar tudo numa vista seria demasiado complexo
  \option Existem vários intervenientes no processo de desenvolvimento
  e temos que ter uma vista para cada um deles
  \option Existem três tipos de vista e normalmente temos uma vista de
  cada tipo
  \option Temos que ter pelo menos uma vista por cada componente do
  sistema, e os sistemas normalmente têm mais do que um componente
 \end{options}
\end{ClosedQuestion}
...
\begin{ClosedQuestion}
  ...
  software é o conceito de \emph{architectural driver}.  Os
  \emph{architectural drivers} são
  ...
  \option Todos os atributos de qualidade capturados em cenários
...
\begin{ClosedQuestion}
  Suponha que no processo de desenho da arquitectura de software de um
  sistema chega à conclusão de que existem relações de utilização
  (\emph{uses}) nos dois sentidos entre quase todos os módulos do
  sistema.  Isto significa que

  \begin{options}
    \option Há muita comunicação entre os vários módulos, o que irá
    provocar um baixo desempenho do sistema
    \option Não é possível instalar o sistema em mais do que uma máquina
    \option Não é possível desenvolver e testar o sistema de forma incremental
    \option É muito difícil explicar o que é que o sistema faz, porque
    precisamos de perceber todos os fluxos de execução
  \end{options}
\end{ClosedQuestion}
...
%% 6
\begin{ClosedQuestion}
  Supondo que se encontra na situação indicada na pergunta anterior,
  ...
  \begin{options}
    ...
    \option Alterava a decomposição feita para o sistema, de modo a
    obter uma nova modularidade que não envolva tantas relações de
    utilização
    \option Alterava algumas das estruturas de dados e dos algoritmos
    escolhidos para minimizar a necessidade de comunicação entre os
    vários módulos
    \option Utilizava o estilo de Generalização para reduzir as
...
%% 7
\begin{ClosedQuestion}
  Foi-lhe pedido para desenvolver uma aplicação que recolha notícias
  de várias fontes de notícias existentes na web e processe essa
  informação para a apresentar de forma resumida aos utilizadores.  As
  várias fontes de notícias providenciam essencialmente o mesmo tipo
  de informação, mas são independentes entre si, apresentando
  diferentes interfaces para aceder às notícias que providenciam.
  Para além disso, é provável que essas interfaces vão sofrendo
  ...
  \begin{options}
    ...
    a comunicação entre os vários componentes
    \option O estilo Cliente-Servidor (\emph{Client-Server}) para
    representar os pedidos feitos pela nova aplicação às várias fontes
    de notícias
    \option O estilo Utilização (\emph{Uses}) para representar as
    relações de dependência existentes entre a aplicação e cada uma
    das fontes de onde esta obtém notícias
    \option O estilo Camadas (\emph{Layers}) para criar uma máquina
    virtual que esconda do resto da aplicação a variabilidade no
    acesso às várias fontes de notícias
 \end{options}
\end{ClosedQuestion}
...
%% 8
\begin{ClosedQuestion}
  Utilizando ainda o exemplo da aplicação da pergunta anterior, qual
  das seguintes afirmações está mais correcta

  \begin{options}
    \option O estilo Generalização não tem utilidade neste caso
    \option O estilo Generalização é útil para representar a herança de
    implementação existente entre os vários módulos
    \option O estilo Generalização é útil para definir uma interface
    comum a todos os módulos que acedem a fontes de notícias
    \option A utilização ou não utilização do estilo Generalização
    neste caso não tem qualquer influência na decomposição do sistema
 \end{options}
\end{ClosedQuestion}
...
\begin{ClosedQuestion}
  ...
  decomposição de um sistema em módulos: a decomposição funcional e a
  decomposição por objectos.  Qual das duas deve ser usada ao efectuar
  a decomposição de um sistema?

  \begin{options}
    \option A decomposição funcional, visto que permite capturar e
    explicar melhor as funcionalidades do sistema aos diferentes
    intervenientes
    \option A decomposição por objectos, visto que é uma abordagem
    mais recente que veio substituir a decomposição funcional,
    trazendo claras vantagens de modularidade para os sistemas
    \option Deve ser usada a decomposição que permitir encapsular
    melhor as mudanças previstas para o sistema
    \option Devem ser usadas ambas as formas de decomposição,
...
\begin{ClosedQuestion}
  ...
  servidores de email, cujo objectivo é permitir efectuar alterações
  aos emails recebidos pelo servidor (por exemplo, para remover
  potenciais vírus ou URLs para sites de phishing).  O objectivo é que
  o servidor de email passe cada email recebido por este sistema antes
  de o processar (quer reencaminhá-lo para outro servidor, ou
  armazená-lo localmente).  Pretende-se que o sistema desenvolvido
  seja facilmente modificável para suportar novos tipos de
  transformações aos emails.  Que estilo arquitectural lhe parece mais
...
\begin{ClosedQuestion}
  ...
  colocação em funcionamento do sistema.  Que estilo arquitectural é
  útil para esta tarefa?

  \begin{options}
    \option O estilo Atribuição de Trabalho (\emph{Work assignment})
    \option O estilo Instalação (\emph{Deployment})
    \option O estilo Utilização (\emph{Uses})
...
\begin{ClosedQuestion}
  ...
  base de dados alguma informação introduzida pelos utilizadores, e
  que um dos requisitos é que essa informação seja mantida
  confidencial, não podendo ser vista sequer pelo administrador da
  base de dados.  Apenas os utilizadores que introduziram essa
  informação devem poder aceder a essa informação novamente.

  Como é que satisfazia este requisito?

  \begin{options}
    \option Não é possível satisfazer este requisito relativamente ao
    administrador da base de dados
    \option Utilizava mecanismos de autenticação robustos para
    identificar os utilizadores do sistema com confiança
    \option Cifrava a informação na base de dados com uma password
    conhecida apenas pela aplicação
    \option Cifrava a informação no cliente web (\emph{browser}) usado
...
%% 13
\begin{ClosedQuestion}
  Utilizando ainda o exemplo da aplicação da pergunta anterior,
  suponha agora que existe também o requisito de que, mesmo na
  eventualidade de existir uma avaria no computador que armazena os
  dados da aplicação, deve ser possível recuperar o funcionamento da
  aplicação com pelo menos todos os dados existentes duas horas antes
  da avaria ter ocorrido.  O tempo de recuperação não é relevante.

  Que estilo arquitectural permite demonstrar melhor a satisfação
  deste requisito?
  ...
  \begin{options}
    ...
    armazena os dados
    \option O estilo Instalação (\emph{Deployment})
    \option O estilo Implementação (\emph{Implementation})
    \option O estilo Repositório (\emph{Repository})
  \end{options}
\end{ClosedQuestion}
...
%% 14
\begin{ClosedQuestion}
  Uma forma de aumentar a disponibilidade de uma aplicação web é
  ...
  \begin{options}
    \option Basta apresentar uma vista de instalação
    (\emph{deployment}) do sistema que contenha o \emph{load-balancer}
    fisicamente entre as máquinas que executam os clientes web e as
    máquinas que executam os servidores
    ...
    modo a que tenha a semântica providenciada pelo \emph{load-balancer}
    \option Cria-se um módulo na vista decomposição que tenha a
...
\begin{ClosedQuestion}
  ...
  os pedidos.  Isto é, processam cada pedido numa nova (ou
  reutilizada) \emph{thread} dentro do mesmo processo, ao invés de
  ...
  funcionamento deste tipo de servidores web, que vista é mais
  adequado?

  \begin{options}
    \option Uma vista do tipo Módulo
    \option Uma vista do tipo Componente-e-Conector
    \option Uma vista do tipo Afectação
    \option Uma vista de Atribuição de Trabalho (\emph{Work assignment})
  \end{options}
\end{ClosedQuestion}
...
%% 16
\begin{ClosedQuestion}
  O padrão Transaction Script é muitas vezes utilizado no
  desenvolvimento de aplicações empresariais porque

  \begin{options}
    \option Permite maior independência no desenvolvimento de
    diferentes casos de uso
    \option É necessário sempre que as operações de negócio têm que
    ser processadas dentro de uma transacção
    \option Complementa bem o padrão Data Mapper no acesso aos dados
    \option Todas as anteriores
  \end{options}
\end{ClosedQuestion}
...
%% 17
\begin{ClosedQuestion}
  Suponha que está a desenvolver uma aplicação para fazer a gestão de
  tarefas de uma equipa.  Nessa aplicação precisa de representar
  informação sobre a duração expectável de cada tarefa (por exemplo, 2
  horas, 3 dias, ou 4 semanas), bem como o instante de tempo em que
  uma determinada tarefa começou ou deve terminar.  Para além disso,
  ...
  contexto de uma determinada tarefa.  Qual é a melhor forma de
  representar estes conceitos temporais no modelo de domínio da
  aplicação?
...
%% 18
\begin{ClosedQuestion}
  Uma organização possui já várias aplicações que acedem a uma base de
  dados relacional, central à organização, e que contém todos os dados
  acumulados ao longo dos últimos 20 anos de funcionamento dos
  sistemas de informação dentro da organização.

  Pretende-se agora desenvolver uma nova aplicação em Java que utiliza
  o padrão Domain Model para representar as entidades de domínio da
  aplicação, a maior parte das quais (mas não todas) correspondem a
  dados já existentes na base de dados central da organização e que lá
  se devem manter.  A generalidade desses dados são manipulados
  concorrentemente pelas aplicações já existentes.

  Que padrão de acesso a dados acha mais adequado para esta nova
  aplicação?
...
%% 19
\begin{ClosedQuestion}
  O padrão Unit of Work é normalmente usado para implementar um Data
  Mapper porque

  \begin{options}
    \option Reduz o número de round-trips à base de dados, melhorando
    assim o desempenho
    \option É responsável por saber qual é a correspondência existente
    entre uma classe que representa uma entidade de domínio e a
    estrutura da base de dados
    \option Garante que cada entidade de domínio é carregada da base
    de dados no máximo uma vez
    \option Reduz o volume de dados transferidos por cada pedido feito
    à base de dados, reduzindo assim a memória necessária para a aplicação
  \end{options}
\end{ClosedQuestion}
...
%% 20
\begin{ClosedQuestion}
  Considere novamente a aplicação que recolhe notícias descrita na
  pergunta~7.  Durante o desenvolvimento desta aplicação descobriu que
  algumas das fontes de notícias utilizadas pela sua aplicação
  comunicam elas próprias entre si para obter notícias de outras
  fontes e providenciá-las juntamente com as suas.  Felizmente, as
  várias fontes utilizam um identificador único para as notícias que
  preservam quando obtêm uma notícia de outra fonte.  Que padrão pode
  utilizar para eliminar notícias repetidas obtidas através das várias
  fontes?

  \begin{options}
    \option O padrão Unit of Work
    \option O padrão Identity Map
    \option O padrão Lazy Load
    \option O padrão Active Record
  \end{options}
\end{ClosedQuestion}
