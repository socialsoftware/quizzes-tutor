...
@DataJpaTest
class FindQuestionByKeyTest extends SpockTest {

    def "simple multiple choice question lookup by key"() {
        given:
        ...
        when:
        def result = questionService.findQuestionByKey(question.getKey())
    ...
    def "simple code fill in question lookup by key"() {
        given:
    ...
    @Unroll
    def "lookup by id of non existent/valid keys (#nonExistentId)"(Integer nonExistentId) {
        when:
        questionService.findQuestionByKey(nonExistentId)
