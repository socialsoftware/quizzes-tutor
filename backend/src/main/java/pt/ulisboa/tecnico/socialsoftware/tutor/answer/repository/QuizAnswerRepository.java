...
public interface QuizAnswerRepository extends JpaRepository<QuizAnswer, Integer> {
    ...
    @Query(value = "SELECT qa.quiz.id FROM QuizAnswer qa JOIN qa.quiz q WHERE qa.user.id = :userId AND q.courseExecution.id = :executionId AND (qa.completed = true OR (q.type = 'IN_CLASS' AND q.creationDate IS NOT NULL))")
