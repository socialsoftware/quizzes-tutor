<template>
  <div class="code-order-create">
    ...
    <v-card-actions>
        ...
        <v-tooltip top>
          <template v-slot:activator="{ on }">
            <v-btn color="primary" small @click="newSlot" v-on="on">New Slot</v-btn>
          </template>
          <span>
    ...
    <draggable tag="div"  @end="endedReorder()" :list="sQuestionDetails.codeOrderSlots" class="list-group" handle=".handle">
      <div 
...
export default class CodeOrderCreate extends Vue {
  ...
  updateList(){
    this.sQuestionDetails.codeOrderSlots = this.sQuestionDetails.codeOrderSlots.sort((a,b) => {
  ...
  endedReorder(){
    this.sQuestionDetails.codeOrderSlots.forEach((element: CodeOrderSlot, index: number)=> {
      element.order = element.order ? index : element.order;
    });
  }
