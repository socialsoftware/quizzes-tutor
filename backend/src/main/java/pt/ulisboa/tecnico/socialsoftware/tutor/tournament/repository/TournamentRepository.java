...
public interface TournamentRepository extends JpaRepository<Tournament, Integer> {
    @Query(value = "SELECT t.* FROM tournaments t WHERE t.course_execution_id = :courseExecutionId", nativeQuery = true)
    List<Tournament> getTournamentsForCourseExecution(Integer courseExecutionId);

    @Query(value = "SELECT t.* FROM tournaments t WHERE t.start_time < :now AND t.end_time > :now AND t.is_canceled = false AND t.course_execution_id = :courseExecutionId", nativeQuery = true)
    List<Tournament> getOpenedTournamentsForCourseExecution(Integer courseExecutionId, LocalDateTime now);

    @Query(value = "SELECT t.* FROM tournaments t WHERE t.end_time < :now AND t.is_canceled = false AND t.course_execution_id = :courseExecutionId", nativeQuery = true)
