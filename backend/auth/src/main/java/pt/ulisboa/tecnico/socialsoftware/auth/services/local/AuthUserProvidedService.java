...
public class AuthUserProvidedService {
    ...
    public AuthDto fenixAuth(FenixEduInterface fenix) {
        ...
        if (authUser.getUserSecurityInfo().isTeacher()) {
            return authUser.getAuthDto(JwtTokenProvider.generateToken(authUser), fenix.getPersonTeachingCourses(),
                    courseExecutionList);
        } else {
    ...
    private void updateTeacherCourses(AuthTecnicoUser authUser, List<CourseExecutionDto> fenixTeachingCourses,
        ...
        if (!fenixTeachingCourses.isEmpty() && authUser.getUserSecurityInfo().isTeacher()) {
            ...
            List<CourseExecutionDto> courseExecutionDtoList = activeTeachingCourses.stream()
                    .filter(courseExecutionDto -> !authUser.getUserCourseExecutions()
                            .contains(courseExecutionDto.getCourseExecutionId()))
    ...
    private void updateAuthUserCourseExecutions(AuthUser authUser, List<CourseExecutionDto> courseExecutionDtoList,
        ...
        updateCourseExecutionsSaga(authUser, authUser.getUserSecurityInfo().getId(), ids, courseExecutionDtoList,
                email);
    ...
    public void updateCourseExecutionsSaga(AuthUser authUser, Integer userId, String ids,
        ...
        // UpdateCourseExecutionsSagaData data = new
        // UpdateCourseExecutionsSagaData(authUser.getId(), userId, ids,
    ...
    public ExternalUserDto registerExternalUserTransactional(Integer courseExecutionId,
        ...
        String username = externalUserDto.getUsername() != null ? externalUserDto.getUsername()
                : externalUserDto.getEmail();
        ...
        if (authUser == null) {
            authUser = (AuthExternalUser) createAuthUser(externalUserDto.getName(), username,
                    externalUserDto.getEmail(),
    ...
    public ExternalUserDto confirmRegistrationTransactional(ExternalUserDto externalUserDto) {
        ...
        try {
            ...
            if (e.getErrorMessage().equals(ErrorMessage.EXPIRED_CONFIRMATION_TOKEN)) {
                ...
                throw new TutorException(e.getErrorMessage());
        }
    ...
    public void confirmRegistrationSaga(AuthUser authUser, Integer authUserId, Integer userId, String password) {
        ...
        // ConfirmRegistrationSagaData data = new
        // ConfirmRegistrationSagaData(authUserId, userId, password);
    ...
    public NotificationResponse<CourseExecutionDto> registerListOfUsersTransactional(InputStream stream,
            int courseExecutionId) {
        Notification notification = new Notification();
        extractUserDtos(stream, notification)
                .forEach(userDto -> registerExternalUserTransactional(courseExecutionId, userDto));
    ...
    public List<CourseExecutionDto> getActiveTecnicoCourses(List<CourseExecutionDto> courses) {
        return courses.stream()
                .map(courseDto -> authRequiredService.getCourseExecutionByFields(courseDto.getAcronym(),
    ...
    public AuthUser createAuthUserSaga(String name, String username, String email, Role role, AuthUserType type,
        ...
        // CreateUserWithAuthSagaData data = new
        // CreateUserWithAuthSagaData(authUser.getId(),
