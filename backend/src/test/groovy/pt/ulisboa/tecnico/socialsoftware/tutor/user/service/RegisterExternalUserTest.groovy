...
class RegisterExternalUserTest extends SpockTest {
    ...
    def "the course execution exists, the external user is enrolled in it and tries to enroll again"() {
        ...
        externalUserDto.setUsername(USER_1_USERNAME)
        externalUserDto.setEmail(USER_1_EMAIL)
        externalUserDto.setRole(User.Role.STUDENT)
        and: "an already created user"
        def user = new Student(USER_1_NAME, USER_1_USERNAME, USER_1_EMAIL, false, AuthUser.Type.EXTERNAL)
    ...
    @Unroll
    def "invalid arguments: executionType=#executionType | username=#username | email=#email | role=#role"() {
        ...
        where:
        executionType        | username        | email           | role                 || errorMessage
        Course.Type.TECNICO  | USER_1_USERNAME | USER_1_EMAIL    | User.Role.STUDENT    || ErrorMessage.COURSE_EXECUTION_NOT_EXTERNAL
        Course.Type.EXTERNAL | null            | USER_1_EMAIL    | User.Role.STUDENT    || ErrorMessage.INVALID_AUTH_USERNAME
        Course.Type.EXTERNAL | "  "            | USER_1_EMAIL    | User.Role.STUDENT    || ErrorMessage.INVALID_AUTH_USERNAME
        Course.Type.EXTERNAL | USER_1_USERNAME | null            | User.Role.STUDENT    || ErrorMessage.INVALID_EMAIL
        Course.Type.EXTERNAL | USER_1_USERNAME | ""              | User.Role.STUDENT    || ErrorMessage.INVALID_EMAIL
        Course.Type.EXTERNAL | USER_1_USERNAME | "test.mail.com" | User.Role.STUDENT    || ErrorMessage.INVALID_EMAIL
        Course.Type.EXTERNAL | USER_1_USERNAME | "test@"         | User.Role.STUDENT    || ErrorMessage.INVALID_EMAIL
        Course.Type.EXTERNAL | USER_1_USERNAME | USER_1_EMAIL    | null                 || ErrorMessage.INVALID_ROLE
        Course.Type.EXTERNAL | USER_1_USERNAME | USER_1_EMAIL    | User.Role.ADMIN      || ErrorMessage.INVALID_ROLE
        Course.Type.EXTERNAL | USER_1_USERNAME | USER_1_EMAIL    | User.Role.DEMO_ADMIN || ErrorMessage.INVALID_ROLE
