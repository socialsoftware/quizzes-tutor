...
public interface QuestionSubmissionRepository extends JpaRepository<QuestionSubmission, Integer> {
    @Query(value = "select * from {h-schema}question_submissions s where s.submitter_id = :submitterId and s.course_execution_id = :courseExecutionId", nativeQuery = true)
    List<QuestionSubmission> findQuestionSubmissionsByUserAndCourseExecution(Integer submitterId, Integer courseExecutionId);

    @Query(value = "select * from {h-schema}question_submissions s where s.course_execution_id = :courseExecutionId", nativeQuery = true)
    List<QuestionSubmission> findQuestionSubmissionsByCourseExecution(Integer courseExecutionId);

    @Query(value = "select * from {h-schema}question_submissions s where s.question_id = :questionId", nativeQuery = true)
    QuestionSubmission findQuestionSubmissionByQuestionId(Integer questionId);

    @Query(value = "select question_id from {h-schema}question_submissions s where s.id = :questionSubmissionId", nativeQuery = true)
    Optional<Integer> findQuestionIdByQuestionSubmissionId(Integer questionSubmissionId);

    @Query(value = "select course_execution_id from {h-schema}question_submissions s where s.id = :questionSubmissionId", nativeQuery = true)
    Integer findCourseExecutionIdByQuestionSubmissionId(Integer questionSubmissionId);

    @Query(value = "select submitter_id from {h-schema}question_submissions s where s.id = :questionSubmissionId", nativeQuery = true)
