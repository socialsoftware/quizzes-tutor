...
class FenixAuthTest extends SpockTest {
    ...
    def "teacher has courses"() {
        given: 'a teacher'
        def user = new User(USER_1_NAME, USER_1_USERNAME, USER_1_EMAIL, User.Role.TEACHER, true, false, AuthUser.Type.TECNICO)
    ...
    def "student does not have courses, throw exception"() {
        given: 'a student'
        def user = new User(USER_1_NAME, USER_1_USERNAME, USER_1_EMAIL, User.Role.STUDENT, true, false, AuthUser.Type.TECNICO)
        userRepository.save(user)
        user.setKey(user.getId())
    ...
    def "student has courses but not in the database, throw exception"() {
        ...
        given: 'a student'
        def user = new User(USER_1_NAME, USER_1_USERNAME, USER_1_EMAIL, User.Role.STUDENT, true, false, AuthUser.Type.EXTERNAL)
        userRepository.save(user)
        user.setKey(user.getId())
